...
public:
	...
	~Object();
	void printID();
	void setID(char *str) { ID = Helper::strCopy(str); }
	char *getID() { return ID; }
	virtual void printName() { printID(); };
...
public:
	...
	void incNoOfLikedPages() { noOfLikedPages++; }
	char **getTempID() { return tempID; }
...
public:
	Page();
	~Page();
	void printName() { cout << title; }
	void incNoOfLikes() { likeCount ++; }
	void setTitle(char *str) { title = Helper::strCopy(str); }
...
public:
	...
	Object *getCommentator() { return commentator; }
	void printName() {};
};
...
public:
	...
	void setDescription(char *des) { description = Helper::strCopy(des); }
	char *getDescription() { return description; }
	void setDate(Date date) { postedDate = date; }
	Date getDate() { return postedDate; }
	Comment **getComment() { return comments; }
	int getCommentCount() { return commentCount; }
	int postedDay() { return postedDate.getDay(); }
	void setComment(Comment **);
	void setCommentCount(int a) { commentCount = a; }
	Object *getShared() { return shared; }
	int getSharedCount() { return shareCount; }
	void showLiked();
...
public:
	...
	static int getCurrentDay() { return currentDate.getDay(); };
	void Search(char *);
};
...
int Helper::returnIndex(char *str)
{
...
User::User()
	:Object('\0'),firstName('\0'),lastName('\0'), noOfFriends(0), noOfLikedPages(0),timeLine(0),noOfPosts(0),friendList(0),likedPages(0)
{}
...
Page::Page()
	:Object(0),title(0),likeCount(0),timeLine(0),noOfPost(0),likingUser(0)
{}
...
Post::Post()
	:Object(0),description(0),commentCount(0),likeCount(0),shareCount(0),liked(0),shared(0),comments(0)
{}
...
Comment::Comment()
	:Object(0),text(0),commentator(0)
{}

Comment::Comment(char * _text, Object *_commentator)
{
...
Facebook::Facebook()
	:allUsers(0),currentUser(0),allPages(0),allPosts(0)
{}
...
{
	char buffer[80];
	inp.open("Project-SocialNetworkPages.txt");
	if (!inp)
...
{
	char buffer[120];
	inp.open("Project-SocialNetworkPosts.txt");
	if (!inp)
		cout << "ERROR IN FILE OPENING\n";
	else
	{
...
{
	char buffer[80];
	inp.open("Project-SocialNetworkUsers.txt");
	if (!inp)
		cout << "ERROR IN FILE OPENING\n";
	else
	{
...
{
	char buffer[180];
	inp.open("Project-SocialNetworkComments.txt");
	if (!inp)
		cout << "ERROR IN FILE OPENING\n";
	else
	{
...
void Facebook::setCurrentUser(char *ID)
{
	int flag = 0;
...
void Facebook::Search(char *str)
{
	...
	for (int i = 0; i < 11; i++)					// I have hard coded 11 because the value of totalPosts gets updated and as Ma'am you have said that							
